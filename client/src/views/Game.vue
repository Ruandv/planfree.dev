<template>
  <div>
    ...
    <Modal v-if="modal" title='display_name_title' @completed="enteredName"></Modal>
    <Sharing v-if="showShareModal" title='share_modal_title' subTitle='share_modal_subtitle' @share="shareGame"></Sharing>
    <Settings v-if="settings" @saveSettings="saveSettings"></Settings>
    <div v-if="!modal && !settings && !showShareModal"   class="home">
      ...
      <div class="options" v-if="!showVotes || (showVotes && countdown !== 0)">
        <button v-for="vote in gameFormat?.values" :key="`vote-${vote}`" class="fib-button"
          :class="{ current: currentVote === vote }" @click="performVote(vote)"
...
function saveSettings(gameType) {
